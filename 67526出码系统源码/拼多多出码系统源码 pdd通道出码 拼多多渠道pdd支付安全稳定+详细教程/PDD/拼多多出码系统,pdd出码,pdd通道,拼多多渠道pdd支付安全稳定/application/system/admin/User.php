...
class User extends Admin
{
    ...
    public function index($q = '')
    {
        if ($this->request->isAjax()) {
            ...
            $data['data'] = UserModel::with('role,client')->where($where)->page($page)->limit($limit)->select();
            foreach ($data['data'] as $row) {
                if (! $row->client) {
                    $client = Client::with('admin')->get($row->c_id);
                    if ($client) {
                        $row['parent_nick'] = '<span style="color:green;">' . $client->nick . '</span>';
                    }
                } else {
                    $row->nick = '<span style="color:green;">' . $row->nick . '<span>';
                }
            }
    ...
    public function addUser()
    {
        if ($this->request->isPost()) {
            ...
            if (ROleModel::getClientRole()->id == $data['role_id']) {
                $client = Client::generator();
                $is_generator = true;
            } else {
                $client = Client::get(['admin_uid' => $data['c_id']]);
            }
        ...
        $this->assign('clientOptions', UserModel::getClientOption());
        $clientRoleId = 0;
        if ($clientRole = ROleModel::getClientRole()) {
            $clientRoleId = $clientRole->id;
        }
        $this->assign('role_id', $clientRoleId);
    ...
    public function editUser($id = 0)
    {
        ...
        $row = UserModel::where('id', $id)->field('id,username,role_id,nick,email,mobile,auth,status,c_id')->find()->toArray();
        if (!$row['auth']) {
        ...
        $this->assign('clientOptions', UserModel::getClientOption($row['c_id']));
        $clientRoleId = 0;
        if ($clientRole = ROleModel::getClientRole()) {
            $clientRoleId = $clientRole->id;
        }
