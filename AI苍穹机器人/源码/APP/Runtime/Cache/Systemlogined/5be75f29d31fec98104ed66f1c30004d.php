...
<?php if((isset($acc[strtoupper(GROUP_NAME)][strtoupper('Dai')])) or (!empty($_SESSION[C('ADMIN_AUTH_KEY')]))): ?><li <?php if(MODULE_NAME == 'Dai'): ?>class="open"<?php endif; ?>>
	...
	<?php if((isset($acc[strtoupper(GROUP_NAME)][strtoupper('Dai')][strtoupper('gonggao')])) or (!empty($_SESSION[C('ADMIN_AUTH_KEY')]))): ?><li>
			<a href="<?php echo U(GROUP_NAME.'/Dai/gonggao');?>">
				<i class="icon-double-angle-right"></i>
				贷款申请
			</a>
		</li><?php endif; ?>
...
<script type="text/javascript">
	$(function() {
				...
				<div id="page-content" class="clearfix">
					...
					<div class="row-fluid">
						<!--PAGE CONTENT BEGINS HERE-->
							<form class="form-horizontal" name="pinaddHandle"  enctype="multipart/form-data" action="<?php echo U(GROUP_NAME .'/Dai/pinaddHandle');?>" method="post">

								<div class="control-group">
									<label class="control-label" for="number"> 生成数量</label>

									<div class="controls">
										<input type="text" value="" name="number" id="number" placeholder="" onkeyup="value=value.replace(/[^0-9]/g,'')" />
									</div>
								</div>
								<div class="control-group">
									<label class="control-label" for="user"> 赠送会员编号</label>

									<div class="controls">
										<input type="text" value="" name="user" id="user" placeholder=""  />
									</div>
								</div>
		...
		$('form[name="pinaddHandle"]').validate({
					//默认为lable
					errorElement : 'span',
					//失败时先移除成功的class
					validClass : 'success',
		...
		$.validator.addMethod("checkurl",function(value,element,params){
				var checkurl = /^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&]*([^<>])*$/;
				return this.optional(element)||(checkurl.test(value));
			},"格式应为:http://www.xxxx.com/！");				
		</script>
