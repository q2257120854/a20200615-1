				...
				<div id="page-content" class="clearfix">
                       <div class="page-header position-relative">
						   <div style="text-align:left;width:100%;">
						    <a type="button" href="<?php echo U(GROUP_NAME.'/Member/pinAdd');?>" class="btn btn-info btn-small no-border"> <i class="icon-plus icon-white"></i>生成激活码</a>
						    </div>
					   </div>
						<!--PAGE CONTENT BEGINS HERE-->
						<form id="table-searchbar" method="POST" class="form-inline well well-small">
							<div class="row-fluid">&nbsp;&nbsp;激活码:
				                <input type="text" class="input-small" name="pin" value="<?php echo ($num); ?>">    
		                        &nbsp;&nbsp;
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
		                                    ...
		                                    <td><?php echo (date('Y-m-d H:i:s',$v["sc_date"])); ?></td>
                                            <td><?php if($v["sy_user"] == 0): ?>未使用<?php else: echo ($v["sy_user"]); endif; ?></td>
                                            <td><?php if($v["zt"] == 0): ?>未使用<?php else: echo (date('Y-m-d H:i:s',$v["sy_date"])); endif; ?></td>										
											<td>
											<a onclick="if(confirm('删除后不可恢復！您确定要删除此激活码吗?')==false)return false;" href="<?php echo U(GROUP_NAME .'/Member/deletePin',array('id'=>$v['id']));?>"><font color=red>删除</font></a>       </td>
