    ...
    return {
        ...
        fixScreen: function (enforce) {
            ...
            if (!metaEl) { // REM
                ...
                refreshRem();
            } else if ((isMobile && !matchScale && (matchWidth && matchWidth[1] != 'device-width')) || enforce) { // 定宽
                var width = parseInt(matchWidth[1]),
                    ...
                    scale = w / width;
                    if (scale < 1) {
                        scale = Math.max(iw, ow, sw, saw) / width;
                    }

                //if (scale < 1) {
                metaEl.content = matchWidth[0] + ',' + fillScale(scale);
                //}
            }
        ...
        formatUcBrowser:function() {
            ...
            if (isUc) {
                $('.yibulocked')
                    .each(function() {
                        var $view = $(this);
                        var top = parseInt($view.css('top'), 10);
                        var left = parseInt($view.css('left'), 10);
                        var right = parseInt($view.css('right'), 10);
                        var bottom = parseInt($view.css('bottom'), 10);
                        if (top <= 150 && bottom === 0 && (left <= 150 || right <= 150)) {
                            var halfHeight = parseInt($view.css('height'),10) / 2 * -1;
                            halfHeight = halfHeight + top;
                            $view.css({ 'top': '50%', 'margin-top': halfHeight + 'px' });
                        }
                    });
            }
        }
