...
$user = $_POST['WIDout_trade_no'];//提交的用户  也可以去session或者cookie中取登录用户 $_SESSION['uid']
$pay_id = $user; //网站唯一标识 需要充值的用户名，用户ID或者自行创建订单 建议传递用户的ID


$price = (float)$_POST["WIDtotal_fee"]; //提交的价格

$param = $_POST['WIDsubject']; //自定义参数  可以留空 传递什么返回什么 用于区分游戏分区或用户身份
...
if($_POST['sign']){ //来自代理网关 验证签名
    ...
    $parameter = array(
        ...
        "return_url" => 'http://'.$_SERVER['HTTP_HOST'].'/daili_pay/return.php',//付款后附带加密参数跳转到该页面
        "notify_url" => 'http://'.$_SERVER['HTTP_HOST'].'/daili_pay/notify.php',//付款后通知该页面处理业务
