define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'template'], function ($, undefined, Backend, Table, Form, Template) {
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: Config.fastadmin.api_url + '/index.php/news/index/addon',
                    add_url: '',
                    edit_url: '',
                    del_url: '',
                    multi_url: ''
                ...
                if (!userinfo) {
                    Layer.open({
                        ...
                        yes: function (index, layero) {
                            Fast.api.ajax({
                                url: Config.fastadmin.api_url + '/index.php/news/Index/user/login/1',
                                dataType: 'jsonp',
                                data: {
                        ...
                        success: function (layero, index) {
                            $(".layui-layer-btn1", layero).prop("href", "http://www.jingxiutequn.com").prop("target", "_blank");
                        }
                    });
                } else {
                    Fast.api.ajax({
        ...
        api: {
            formatter: {
                ...
                price: function (value, row, index) {
                    return parseFloat(value) == 0 ? '<span class="text-success">内部渠道</span>' : '<span class="text-danger">￥' + value + '</span>';
                },
