...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar','amount', 'point'];
...
/**
    * 变更会员积分
     ...
     */
    public function point($point,$user_id,$dec,$memo)
    {
     return User::point($point,$user_id,$dec,$memo);
    }
    /**
     * 根据Token初始化
     *
     ...
     * @return boolean
     */
    public function register($username, $password, $fatherid = 0, $mobile = '',$point=0, $extend = [])
    {
     ...
     * @param string    $password   密码
     * @param boolean   $ismd5      是否加密
     * @return boolean
     */
    public function login($account, $password,$ismd5=1)
    {
        $field = Validate::is($account, 'email') ? 'email' : (Validate::regex($account, '/^1\d{10}$/') ? 'mobile' : 'openid');
        ...
        if ($ismd5){
            $str_md5=$this->getEncryptPassword($password, $user->salt);
        } else {
            $str_md5=$password;
        }
