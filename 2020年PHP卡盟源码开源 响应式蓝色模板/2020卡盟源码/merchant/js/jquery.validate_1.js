...
$.extend($.validator, {
	...
	messages: {
		...
		number: "请填写合适的数字.",
		digits: "只能输入数字.",
		linkname: "请输入2-4位中文姓名",
		moble:"请输入正确的手机号码",
		creditcard: "Please enter a valid credit card number.",
		equalTo: "两次密码输入不一致.",
		accept: "文件格式不对！",
		idcade:"请填写正确的身份证号",
		loginname: "用户名长度必须为6-20位的数字或字母",
	...
	//	},
		...
		//新的验证电子邮件
		email:function(value,element){
			return this.optional(element) || /^[\w-]+(\.[\w-]+)*@([\w-]+\.)+(com|cn)$/.test(value);
			
		},
		
		//验证身份证号
		idcade:function(value,element){
			return this.optional(element) || /^(\d{6})(18|19|20)?(\d{2})([01]\d)([0123]\d)(\d{3})(\d|X)?$/.test(value);
			
		},
		
		//验证中文姓名
		linkname:function(value,element){
			
			return this.optional(element) || /^[\u0391-\uFFE5]+$/.test(value);
		},
		//验证手机号
		moble:function(value,element){
