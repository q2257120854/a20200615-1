...
$nav=$_REQUEST['nav'];
if     ($mid==1){
$search="where LagID=1";
}elseif($mid==2){
$search="where  LagID=2 and PartID='$nav'";
}
...
mysql_query("delete from product_class  where RootID='$_REQUEST[Id]'",$conn1); 
...
mysql_query("delete from product_class  where NumberID='$_REQUEST[Id]'",$conn1); 
mysql_query("delete from product_class  where PartID='$_REQUEST[Id]'",$conn1); 
...
{ 
$godo=mysql_query("update product_class set open='0' where id='$value'",$conn1); 
}
echo "<script>alert('提交成功!');;self.location=document.referrer;</script>";
}
...
foreach($allArray as $value) 
{ 
$godo=mysql_query("update product_class set open='1' where id='$value'",$conn1); 
}
echo "<script>alert('提交成功!');;self.location=document.referrer;</script>";
}
...
<ul>
<?php
$Rss="SELECT * FROM product_class where LagID=0  order by Classorder asc,id desc limit 0,20 ";
...
?>
<li <?php if ($Orzx['3']==$big) {?>class="hover"<?php } ?>><a href="info_class.php?big=<?=$Orzx['3']?>&sid=<?=$Orzx['0']?>"><?=$Orzx['7']?></a></li>
<?php
$i++;
}
}?>
...
<a href="class_add.php?Id=<?=$big?>"><img src="../images/a1.jpg"></a>
</div>
...
<td width="41%" align="center" class="table_top">目录名称</td>
<td width="16%" align="center" class="table_top">排序</td>
...
<tr>
<td><a href="class_add.php?Action=edit&Id=<?=$Orzx[id]?>" class="a edit1"></a></td>
