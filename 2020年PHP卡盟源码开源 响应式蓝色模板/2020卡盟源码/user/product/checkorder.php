...
####订单
$od_sql=mysql_query("select * from product_order where orderid='$id' and username='$_SESSION[ysk_number]' and docking=0 and sid=0",$conn1);
...
<td colspan="3" align="left" class="tdleft"><?=$ord['zongprice']/$ord['nums']?> <?=$moneytype?> | <?=$ord['nums']?> 个 | <?=$ord['zongprice']?> <?=$moneytype?></td>
</tr>
...
<?php
$Rss="SELECT * FROM details_funds  where orderid like '%$ord[orderid]%'   order by begtime desc,id desc ";
