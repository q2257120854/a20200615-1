...
$ord_sql=mysql_query("select * from product_order where orderid='$id'  and username='$_SESSION[ysk_number]' and docking=0 and sid=0 ",$conn1);  ####订单
...
$pro_sql=mysql_query("select * from product where id='$order[pid]' ",$conn1);       ####产品
$pro=mysql_fetch_array($pro_sql);
...
<body>
<?php if ($Action=='') {?>
<form action="?Action=save&id=<?=$id?>" method="post">
...
<td width="38%" class="tdleft"><?=date("Y-m-d G:i:s",$order['time'])?> / <?php if ($order['begtime']!=''){?><?=date("Y-m-d G:i:s",$order['begtime'])?><?php } ?></td>
</tr>
<tr>
...
<tr>
<td width="14%" height="32" class="tdleri">商品及库存：</td>
<td width="32%" align="left" class="tdleft"><?=$pro['title']?> (<?=$pro['kucun']?>)</td>
...
//-------------订单不处理 不退款情况
if ($yo4=='2'){
mysql_query("update `product_order`  set refund=3  where orderid='$id' and username='$_SESSION[ysk_number]' ",$conn1); ///////更新订单
if ($order['locks']!=0){
mysql_query("update `product_order`  set refund=3  where orderid='$id' and username='$_SESSION[ysk_number]' ",$conn3); ///////更新平台订单
}
//-------------订单处理情况
}else{
