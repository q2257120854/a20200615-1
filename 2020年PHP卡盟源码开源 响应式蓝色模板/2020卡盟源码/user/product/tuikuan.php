...
<td width="31%" class="table_top">商品</td>
<td width="12%" class="table_top">订单编号</td>

<td width="10%" align="center" class="table_top">充值账户</td>
<td width="6%" class="table_top">数量</td>
<td width="14%" class="table_top">购买客户</td>
...
<td><?=$row['nums']?></td>
<td><?=$yx_us['company']?> <span style="color:#666">[<?=$row['number']?>]</span></td>
<td style="text-align:center"><?=date("Y-m-d G:i:s",$row['time'])?></td>
<td><a  href="#art1" onClick="art.dialog.open('/user/product/myorder.php?id=<?=$row[orderid]?>',{ title:'直销平台订单详细记录',width:900,height:400,lock: true,fixed:true});">
<?php        if ($row['refund']=='1') {?>
